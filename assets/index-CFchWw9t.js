...
const useIsOpen = () => {
  const [open, setIsOpen] = reactExports.useState(false);
  const checkIsOpen = reactExports.useCallback(() => {
...
const useBalance = (checkIsOpen) => {
  ...
  reactExports.useEffect(() => {
    const unsubscribe = wallet.balance.subscribeBalance((balance2) => {
      checkIsOpen();
      setBalance(balance2);
    });
    return () => {
...
const RedeemEcash = () => {
  const [ecashInput, setEcashInput] = reactExports.useState("");
  const [redeemResult, setRedeemResult] = reactExports.useState("");
  const [redeemError, setRedeemError] = reactExports.useState("");
...
const SendLightning = () => {
  const [lightningInput, setLightningInput] = reactExports.useState("");
  const [lightningResult, setLightningResult] = reactExports.useState("");
  const [lightningError, setLightningError] = reactExports.useState("");
...
//# sourceMappingURL=index-CFchWw9t.js.map
